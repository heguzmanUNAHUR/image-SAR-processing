...
│   ├── data/                           # Modulos de procesamiento de datos e indices SAR (Fase 1 y 4)
...
│   │   └── multi_fire_selector.py      # Selector dinamico e inspector de poligonos para N -> M (Fase 4)
│   ├── models/                         # Modulos de seleccion de caracteristicas, clasificadores e inferencia (Fase 2, 3 y 4)
...
│   ├── evaluation/                     # Modulos de validacion espacial y evaluacion (Fase 2 y 4)
...
│   └── 04_multi_fire_training.py       # Script orquestador de entrenamiento multi-incendio N -> M y Zero-Shot (Fase 4)
...
│   ├── MULTI_FIRE_TRAINING.md          # Especificacion tecnica del entrenamiento multi-incendio y Zero-Shot (Fase 4)
...
### Fase 2: Entrenamiento y Evaluacion de Modelos ML Monosite
Esta fase abarca la seleccion interactiva de caracteristicas espectrales, la particion espacial estricta por poligonos (**StratifiedGroupKFold**), el entrenamiento de 16 clasificadores de Machine Learning y la evaluacion cuantitativa final sobre un **Conjunto de Prueba Espacialmente Aislado** de un incendio individual.
...
### Fase 4: Entrenamiento Multi-Incendio y Transferencia Cruzada ($N \to M$)
Esta fase extiende la arquitectura para permitir experimentos flexibles de **Transferencia Cruzada** y **Modelos Globales Multi-Incendio**. Permite elegir interactivamente cualquier combinacion de $N$ incendios para entrenamiento y $M$ incendios para evaluacion. Aplica una regla dinamica donde el menor poligono por clase de cada incendio se reserva para Prueba Espacial y todos los sobrantes ($\ge 4$) alimentan el entrenamiento.

- **Documentacion detallada de la Fase 4**: [docs/MULTI_FIRE_TRAINING.md](docs/MULTI_FIRE_TRAINING.md)
- **Script de ejecucion**: `scripts/04_multi_fire_training.py`
...
### 2. Ejecucion de la Fase 2 (Entrenamiento Monosite)
...
### 4. Ejecucion de la Fase 4 (Entrenamiento Multi-Incendio N -> M y Zero-Shot)

```bash
# Modo interactivo (GUI Tkinter de doble columna para seleccion de incendios, indices y modelos)
python scripts/04_multi_fire_training.py

# Modo CLI: Transferencia Cruzada Zero-Shot (Entrenar con Ponton y San Juan, evaluar en Los Alerces)
...
# Modo CLI: Modelo Global Multi-Incendio (Entrenar con todos, evaluar en todos)
